...
def Weight_Loss(age, weight, height, meal):    
    ...
    for ii in range(len(y_pred)):
        if y_pred[ii]==2:
            suggested_items.append(Food_itemsdata[ii])
    return suggested_items
            

def Weight_Gain(age, weight, height, meal):
    data=pd.read_csv('input.csv')
    data.head(5)
    Breakfastdata=data['Breakfast']
    BreakfastdataNumpy=Breakfastdata.to_numpy()
...
def Healthy(age, weight, height, meal):
    data=pd.read_csv('input.csv')
    data.head(5)
    Breakfastdata=data['Breakfast']
    BreakfastdataNumpy=Breakfastdata.to_numpy()
